...
### 3. 系统信息 (SystemInfo) - 客户端信息
- **浏览器信息**：
  - 浏览器名称和版本（Chrome、Edge、Firefox等）
  - 用户代理字符串
  - 语言设置和支持语言列表
  - Cookie启用状态
  - 网络连接状态
...
- **网络信息**：
  - 在线/离线状态
  - 网络类型（4G、WiFi等，如果浏览器支持）
  - 下行速度和往返延迟（如果浏览器支持）
  - 省流模式状态
  - 时区信息
- **时间信息**：本地时间、ISO时间、时间戳、时区偏移
- Web界面：纯客户端实现，无需服务器请求，信息一键复制

**注意**：此工具显示的是访问者（客户端）的系统信息，而非运行服务器的机器信息。所有信息都通过JavaScript浏览器API在客户端获取。
...
##### 系统信息工具（客户端信息）
```bash
# 注意：系统信息工具现在通过Web界面的JavaScript获取客户端信息
# 不再需要通过API调用，直接访问 http://localhost:18787/system_info.html
# 
# 如需通过API获取服务器端系统信息，可以这样调用：
